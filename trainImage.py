...
def getImagesAndLables(path):
    ...
    imagePath = [
        ...
        if f.endswith(('.jpg', '.png'))  # Filter for valid image extensions
    ]
    ...
    for imagePath in imagePath:
        try:
            ...
            Id = int(os.path.split(imagePath)[-1].split("_")[1])  # Extract ID from filename
            faces.append(imageNp)
            Ids.append(Id)
        except Exception as e:
            print(f"Error reading image {imagePath}: {e}")
